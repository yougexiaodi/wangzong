  ...
  <div>
    ...
    <group>
      ...
      <x-input type="tel" class="weui-vcode" placeholder="请输入短信验证码" v-model="sms_code">
        ...
        <x-button v-else-if="minutesCount === 0" slot="right" type="warn" plain mini @click.native="sendMsg">发送验证码
        </x-button>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      sendMsg() {
        this.$http.post(this.getSmsUrl, {mobile: this.mobile, code: this.code, pid: sessionStorage.getItem('pid')})
          .then((res) => {
