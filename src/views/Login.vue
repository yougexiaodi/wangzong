  ...
  <div>
    ...
    <group>
      ...
      >
        ...
        <x-button disabled v-else v-cloak slot="right" type="warn" plain mini
          >{{ minutesCount }}秒后获取</x-button
        >
      </x-input>
    </group>
    <box gap="10px 10px">
      <div class="buy">
        <x-button v-if="(code && mobile && sms_code) === ''" v-cloak disabled
          >输入验证信息</x-button
        >
        <x-button type="warn" v-else v-cloak @click.native="login"
          >立即登录</x-button
        >
      </div>
    </box>
...
export default {
  ...
  methods: {
    getVerify() {
      this.$http.post(this.getVerifyUrl).then((res) => {
