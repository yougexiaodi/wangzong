        ...
        <template v-for="vm in couponList">
          <mu-list-item disabled>
            ...
            <div class="ecard-coupon-list-text">
              ...
              <p style="color: green" v-show="vm.state === '1'" v-model="showCode" @click="qrCode(vm.code)">
                点击生成二维码</p>
              <div v-transfer-dom>
...
<script type="text/babel">
  import {Group, Cell, XButton, Confirm, Toast, Radio, Qrcode, XDialog, TransferDomDirective as TransferDom} from 'vux'
    ...
    methods: {
      ...
      refund(orderId) {
        ...
        self.$vux.confirm.show({
          ...
          content: '退款后本日抢购名额作废',
          onConfirm() {
            self.$http.post(self.refundUrl, {order_id: orderId, pid: sessionStorage.getItem('pid'), wx_id: '2'})
              .then((res) => {
                this.onRefund = false
                if (res.data.status === 0) {
                  self.getDataDetails()
      ...
      getWx() {
        let self = this
        self.$http.post(self.getWxIdUrl, {pid: sessionStorage.getItem('pid'), wx_id: 2, order_id: self.order_id})
          .then((res) => {
            if (res.data.status === 0) {
