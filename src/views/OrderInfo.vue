...
<script>
  ...
  export default {
    ...
    methods: {
      getData () {
        this.$http.post(this.getDataUrl, {order_id: this.$route.query.order_id, pid: sessionStorage.getItem('pid')})
          .then((res) => {
        ...
        this.$http.post(this.loginState, {pid: sessionStorage.getItem('pid')})
          .then((res) => {
            if (res.data.status === 0) {
              ...
              this.$router.push({path: '/login', query: {path: this.$route.path + '?id=' + this.$route.query.id}})
            }
